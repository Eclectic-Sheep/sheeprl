...
def policy_loss(policy: torch.distributions.distribution, batch: TensorDict, clip_coef: float) -> torch.Tensor:
    ...
    Args:
        policy (torch.distributions.distribution): the policy distribution.
    ...
    """
    new_logprobs = policy.log_prob(batch["actions"])
...
def entropy_loss(entropy: Tensor) -> Tensor:
    return entropy.mean()
