...
class NoopResetEnv(gym.Wrapper):
    ...
    def reset(self, **kwargs) -> np.ndarray:
        ...
        for _ in range(noops):
            obs, _, truncated, _, done = self.env.step(self.noop_action)
...
class FireResetEnv(gym.Wrapper):
    ...
    def reset(self, **kwargs) -> np.ndarray:
        self.env.reset(**kwargs)
        obs, _, truncated, _, done = self.env.step(1)
        if truncated or done:
            self.env.reset(**kwargs)
        obs, _, truncated, _, done = self.env.step(2)
        if truncated or done:
            self.env.reset(**kwargs)
        return obs
...
class EpisodicLifeEnv(gym.Wrapper):
    ...
    def step(self, action: int):
        obs, reward, truncated, info, done = self.env.step(action)
        ...
        self.lives = lives
        return obs, reward, truncated, info, done
    ...
    def reset(self, **kwargs) -> np.ndarray:
        ...
        if self.was_real_done:
            ...
            obs, _, truncated, _, done = self.env.step(0)
...
class MaxAndSkipEnv(gym.Wrapper):
    ...
    def step(self, action: int):
        ...
        return max_frame, total_reward, truncated, info, done
