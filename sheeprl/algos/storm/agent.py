...
from sheeprl.models.models import CNN, MLP, DeCNN, MultiDecoder, MultiEncoder, StormGPT
...
class RecurrentModel(nn.Module):
    ...
    def __init__(
        ...
        config = Config(
            ...
            n_layer=2,
            n_head=8,
            n_embd=recurrent_state_size,
            norm_eps=1e-3,
        )
        self.gpt = StormGPT(config=config)
    ...
    def forward(
        ...
        out = self.gpt(feat, input_pos=input_pos, mask=mask, dones=None)
...
class RSSM(nn.Module):
    ...
    def dynamic(
        self, actions: Tensor, embedded_obs: Tensor, is_first: Tensor, mask: Tensor, dones: Tensor
        ...
        Args:
            actions (Tensor): the action taken by the agent.
        ...
        posteriors_logits, posteriors = self._representation(embedded_obs)
        posteriors = posteriors.view(*posteriors.shape[:-2], -1)
        hidden_states = self.recurrent_model(torch.cat((posteriors, actions), -1), mask=mask, dones=dones)
        priors_logits, priors = self._transition(hidden_states)
        priors = priors.view(*priors.shape[:-2], -1)
        return hidden_states, posteriors, priors, posteriors_logits, priors_logits
    ...
    def imagination(self, prior: Tensor, actions: Tensor, index_pos: Optional[Tensor] = None) -> Tuple[Tensor, Tensor]:
        """
        ...
        """
        hidden_state = self.recurrent_model(torch.cat((prior, actions), -1), index_pos=index_pos)
        _, imagined_prior = self._transition(hidden_state)
        return imagined_prior, hidden_state
...
class PlayerDV3(nn.Module):
    ...
    def __init__(
        ...
        self.rssm.recurrent_model.gpt.set_kv_cache(batch_size=num_envs)
        self.actor = actor
        self.device = device
    ...
    def get_exploration_action(
        self, obs: Dict[str, Tensor], mask: Optional[Dict[str, Tensor]] = None, input_pos: Optional[Tensor] = None
    ) -> Tensor:
        """
        ...
        """
        actions = self.get_greedy_action(obs, mask=mask, input_pos=input_pos)
        expl_actions = None
    ...
    def get_greedy_action(
        ...
        _, posterior = self.rssm._representation(embedded_obs)
        posterior = posterior.view(*posterior.shape[:-2], self.stochastic_size * self.discrete_size)
        hidden_state = self.rssm.recurrent_model(torch.cat((posterior, self.actions), -1), input_pos=input_pos)
        actions, _ = self.actor(torch.cat((posterior, hidden_state), -1), is_training, mask)
...
def build_models(
    ...
    representation_model = MLP(
        input_dims=encoder.cnn_output_dim + encoder.mlp_output_dim,
