...
from sheeprl.algos.storm.agent import PlayerDV3, WorldModel, build_models
from sheeprl.algos.storm.loss import reconstruction_loss
from sheeprl.algos.storm.utils import Moments, compute_lambda_values, test
...
def train(
    ...
    # Reset KV-cache
    world_model.rssm.recurrent_model.gpt.clear_kv_cache()
    ...
    # for idx in non_zero:
    #     dim = idx - prev
    #     mask[prev + 1 : prev + dim + 1, prev + 1 : prev + dim + 1] = torch.ones(dim, dim, device=device).tril()
    #     prev = idx
    ...
    latent_states, posteriors, priors, posteriors_logits, priors_logits = world_model.rssm.dynamic(
        batch_actions, embedded_obs, data["is_first"], mask=mask, dones=data["dones"]
    )

    # Compute predictions for the observations
    reconstructed_obs: Dict[str, torch.Tensor] = world_model.observation_model(posteriors)
    ...
    latent_states = latent_states.detach().reshape(1, -1, recurrent_state_size)
    imagined_latent_state = torch.cat((imagined_prior, latent_states), -1)
    ...
    # Set the KV-cache
    world_model.rssm.recurrent_model.gpt.set_kv_cache(batch_size=batch_size * sequence_length)

    # Imagine trajectories in the latent space
    for i in range(1, cfg.algo.horizon + 1):
        imagined_prior, hidden_state = world_model.rssm.imagination(
            imagined_prior, actions, input_pos=torch.as_tensor([i - 1], device=device)
        )
        imagined_prior = imagined_prior.view(1, -1, stoch_state_size)
        imagined_latent_state = torch.cat((imagined_prior, hidden_state), -1)
...
def main(fabric: Fabric, cfg: Dict[str, Any]):
    ...
    for update in range(start_step, num_updates + 1):
            ...
            if (
                ...
                with torch.no_grad():
                    ...
                    real_actions = actions = player.get_exploration_action(
                        preprocessed_obs,
                        mask,
                        input_pos=torch.as_tensor([step_num % cfg.per_rank_sequence_length], device=device),
                    )
        ...
        if update >= learning_starts and updates_before_training <= 0:
            # Save player KV-cache
            mask_cache = (
                None
                if player.rssm.recurrent_model.gpt.mask_cache is None
                else player.rssm.recurrent_model.gpt.mask_cache.clone()
            )
            caches = [copy.deepcopy(block.attn.kv_cache) for block in player.rssm.recurrent_model.gpt.transformer.h]
            ...
            # Re-set the caches
            player.rssm.recurrent_model.gpt.mask_cache = mask_cache
            for i, block in enumerate(player.rssm.recurrent_model.gpt.transformer.h):
                block.attn.kv_cache = caches[i]
