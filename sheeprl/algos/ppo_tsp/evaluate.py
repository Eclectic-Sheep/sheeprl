...
@register_evaluation(algorithms="ppo_tsp")
def evaluate_ppo(fabric: Fabric, cfg: Dict[str, Any], state: Dict[str, Any]):
    ...
    if cfg.algo.cnn_keys.encoder + cfg.algo.mlp_keys.encoder + cfg.algo.graph_keys.encoder == []:
        raise RuntimeError(
            "You should specify at least one CNN keys or MLP keys from the cli: "
            "`cnn_keys.encoder=[rgb]` or `mlp_keys.encoder=[state]`"
        )
    fabric.print("Encoder GNN keys:", cfg.algo.graph_keys.encoder)
