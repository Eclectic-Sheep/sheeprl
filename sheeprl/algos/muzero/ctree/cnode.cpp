...
namespace tree{
    ...
    void CNode::expand(int hidden_state_index_x, int hidden_state_index_y, float reward, const std::vector<float> &policy_logits){
    ...
    float CNode::get_mean_q(int isRoot, float parent_q, float discount){
        ...
        for(int a = 0; a < this->action_num; ++a){
            ...
            if(child->visit_count > 0){
                float qsa = child->reward + discount * child->value();
    ...
    void CRoots::prepare(float root_exploration_fraction, const std::vector<std::vector<float>> &noises, const std::vector<float> &rewards, const std::vector<std::vector<float>> &policies){
        for(int i = 0; i < this->root_num; ++i){
            this->roots[i].expand(0, i, rewards[i], policies[i]);
    ...
    void CRoots::prepare_no_noise(const std::vector<float> &rewards, const std::vector<std::vector<float>> &policies){
        for(int i = 0; i < this->root_num; ++i){
            this->roots[i].expand(0, i, rewards[i], policies[i]);
    ...
    void cbatch_back_propagate(int hidden_state_index_x, float discount, const std::vector<float> &rewards, const std::vector<float> &values, const std::vector<std::vector<float>> &policies, tools::CMinMaxStatsList *min_max_stats_lst, CSearchResults &results){
        for(int i = 0; i < results.num; ++i){
            results.nodes[i]->expand(hidden_state_index_x, i, rewards[i], policies[i]);
    ...
    float cucb_score(CNode *child, tools::CMinMaxStats &min_max_stats, float parent_mean_q, float total_children_visit_counts, float pb_c_base, float pb_c_init, float discount){
