...
cdef extern from "cnode.h" namespace "tree":
    cdef cppclass CNode:
        ...
        void expand(int hidden_state_index_x, int hidden_state_index_y, float reward, vector[float] policy_logits)
    ...
    cdef cppclass CRoots:
        ...
        void prepare(float root_exploration_fraction, const vector[vector[float]] &noises, const vector[float] &rewards, const vector[vector[float]] &policies)
    ...
    void cbatch_back_propagate(int hidden_state_index_x, float discount, vector[float] rewards, vector[float] values, vector[vector[float]] policies,
