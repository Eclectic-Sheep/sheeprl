...
def player(args: PPOPixelContinuousArgs, world_collective: TorchCollective, player_trainer_collective: TorchCollective):
    ...
    single_global_step = int(args.num_envs * args.rollout_steps * args.action_repeat)
    ...
    chunks_sizes = [
        ...
        for chunk in torch.tensor_split(
            torch.arange(single_global_step / args.action_repeat), world_collective.world_size - 1
        )
    ]
        ...
        with Join([agent._forward_module]):
            for _ in range(args.update_epochs):
                for batch_idxes in sampler:
                    ...
                    normalized_obs = batch["observations"] / 255.0 - 0.5
                    _, logprobs, entropy, new_values = agent(normalized_obs, batch["actions"])
                    ...
                    # Entropy loss
                    entropy = entropy_loss(entropy, reduction=args.loss_reduction)
...
def main():
    ...
    parser = HfArgumentParser(PPOPixelContinuousArgs)
    args: PPOPixelContinuousArgs = parser.parse_args_into_dataclasses()[0]
