...
from sheeprl.algos.dreamer_v3.args import DreamerV2Args
from sheeprl.algos.dreamer_v3.utils import compute_stochastic_state, init_weights
from sheeprl.models.models import CNN, MLP, DeCNN
from sheeprl.utils.model import Conv2dSame, LayerNormChannelLast, LayerNormGRUCell
...
class RecurrentModel(nn.Module):
    ...
    def __init__(
        ...
        self.rnn = LayerNormGRUCell(dense_units, recurrent_state_size, layer_norm=layer_norm)
...
class Actor(nn.Module):
    """
    The wrapper class of the dreamer_v3 Actor model.
    ...
    def __init__(
        ...
        self.model = MLP(
            ...
            flatten_dim=None,
            norm_layer=[nn.LayerNorm] * mlp_layers,
            norm_args=[
                {"normalized_shape": dense_units} * mlp_layers,
            ],
        )
...
def build_models(
    ...
    encoder = nn.Sequential(
        CNN(
            ...
            norm_layer=[LayerNormChannelLast] * 4,
            norm_args=[
                {"normalized_shape": 1 * args.cnn_channels_multiplier},
                {"normalized_shape": 2 * args.cnn_channels_multiplier},
                {"normalized_shape": 4 * args.cnn_channels_multiplier},
                {"normalized_shape": 8 * args.cnn_channels_multiplier},
            ],
        ),
        nn.Flatten(-3, -1),
    )
    ...
    recurrent_model = RecurrentModel(np.sum(actions_dim) + stochastic_size, args.dense_units, args.recurrent_state_size)
    ...
    reward_model = MLP(
        ...
        activation=dense_act,
        flatten_dim=None,
        norm_layer=[nn.LayerNorm] * args.mlp_layers,
        norm_args=[
            {"normalized_shape": args.dense_units} * args.mlp_layers,
        ],
    )
