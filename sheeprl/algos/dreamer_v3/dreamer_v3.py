...
def main(cfg: DictConfig):
    ...
    world_model, actor, critic, target_critic = build_models(
        ...
        state["actor"] if cfg.checkpoint_path else None,
        state["critic"] if cfg.checkpoint_path else None,
        state["target_critic"] if cfg.checkpoint_path else None,
    )
    player = PlayerDV3(
        ...
        aggregator = MetricAggregator(
            {
                ...
                "Grads/actor": MeanMetric(sync_on_compute=cfg.metric.sync_on_compute),
                "Grads/critic": MeanMetric(sync_on_compute=cfg.metric.sync_on_compute),
            }
        )
    ...
    for k in obs_keys:
        ...
        obs[k] = torch_obs
    step_data["dones"] = torch.zeros(cfg.num_envs, 1).float()
    step_data["rewards"] = torch.zeros(cfg.num_envs, 1).float()
    ...
    for global_step in range(start_step, num_updates + 1):
        ...
        step_data["actions"] = torch.from_numpy(actions).view(cfg.num_envs, -1).float()
