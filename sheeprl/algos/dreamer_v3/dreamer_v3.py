...
from sheeprl.utils.precision import MixedPrecisionMultiScaler
...
def train(
    ...
    wm_backward_kw = {}
    if isinstance(fabric.strategy.precision, MixedPrecisionMultiScaler):
        wm_backward_kw = {"optim_id": world_optimizer.optim_id}
    fabric.backward(rec_loss, **wm_backward_kw)
    ...
    actor_backward_kw = {}
    if isinstance(fabric.strategy.precision, MixedPrecisionMultiScaler):
        actor_backward_kw = {"optim_id": actor_optimizer.optim_id}
    fabric.backward(policy_loss, **actor_backward_kw)
    actor_grads = None
    ...
    critic_backward_kw = {}
    if isinstance(fabric.strategy.precision, MixedPrecisionMultiScaler):
        critic_backward_kw = {"optim_id": critic_optimizer.optim_id}
    fabric.backward(value_loss, **critic_backward_kw)
    critic_grads = None
...
def main(fabric: Fabric, cfg: Dict[str, Any]):
    ...
    if isinstance(fabric.strategy.precision, MixedPrecisionMultiScaler):
        world_optimizer.optimizer.optim_id = "wm_optim"
        actor_optimizer.optimizer.optim_id = "actor_optim"
        critic_optimizer.optimizer.optim_id = "critic_optim"
        fabric.strategy.precision.setup_scalers(["wm_optim", "actor_optim", "critic_optim"])
    moments = Moments(
